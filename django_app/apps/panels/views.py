...
def test_search(request):
    ...
    for test in Test.objects.filter(name__icontains=request.POST.get("search")):
    ...
    return render(
        request, "panels/partials/test_search.html", {"search_results": search_results}
    )
...
def paneltest_sort(request):
    ...
    for key in form_data.keys():
        if re.match(pattern, key):
            paneltest_rows.append({"order_name": key, "order_value": i})
            i += 1
